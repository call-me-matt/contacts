		...
		<template v-if="!isNoAddressbookAvailable">
			...
			<Modal v-if="isOpened"
				...
				<section class="import-contact__modal-addressbook">
					<h3>{{ t('contacts', 'Update all avatars') }}</h3>
					<Multiselect
				...
				<section class="import-contact__modal-pick">
					...
					<button
						...
						{{ t('contacts', 'From any social network') }}
					</button>
					<button
						...
						{{ t('contacts', 'From selected social network') }}
					</button>
				</section>
			</Modal>
		</template>
